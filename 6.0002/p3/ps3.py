...
class RectangularRoom(object):
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        self.tiles[math.floor(pos.get_x()), math.floor(pos.get_y())] = max(0,
    ...
    def is_tile_cleaned(self, m, n):
        ...
        Note: The tile is considered clean only when the amount of dirt on this
              tile is 0.
        """
        return not self.tiles[(m,n)] #returns True if zero, False if not zero
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        count = 0
        for dirty in self.tiles.values():
            if not dirty: count += 1
        return count
    ...
    def is_position_in_room(self, pos):
        ...
        """
        if self.tiles.get((math.floor(pos.get_x()), math.floor(pos.get_y())), None) == None: return False
        else: return True
...
class EmptyRoom(RectangularRoom):
    ...
    def get_num_tiles(self):
        ...
        Returns: an integer; the total number of tiles in the room
        """
        return self.width * self.height # int * int
...
class FurnishedRoom(RectangularRoom):
    ...
    def add_furniture_to_room(self):
        ...
        for i in range(f_bottom_left_x, f_bottom_left_x + furniture_width):
            for j in range(f_bottom_left_y, f_bottom_left_y + furniture_height):
                if (i,j) not in self.furniture_tiles: #avoid duplicates
    ...
    def get_num_tiles(self):
        ...
        """
        # nowhere is it guaranteed that furnished_tiles doesn't have duplicates so
        # I modiified the add_furniture_to_room() to not add duplicates
        # was originaly gonna use this but its heavy
        # #remove duplicates
        # for i, tile in enumerate(self.furniture_tiles):
        #     for j in range(self.furniture_tiles.count(tile) - 1):
        #         self.pop(self.tiles.index(tile, i+1))
        #         #gets rid of duplicates, so its chill
        return self.width * self.height - len(self.furniture_tiles)
        
    # def while_not_valid(self, x, y):
    #     while (x,y) not in self.furniture_tiles:
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        if self.room.is_position_valid(new_pos):
            ...
            #no need to check if tile is cleaned since I make the capacity check anyways
        else:
...
# furnished room takes forever to finish because root just goes over cleaned tiles
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        """
        if self.gets_faulty(): self.set_robot_direction(random.random() * 360)
        else:
            new_pos = self.pos.get_new_position(self.direction, self.speed)
            if self.room.is_position_valid(new_pos):
                self.set_robot_position(new_pos)
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for i in range(1, num_robots + 1):
        robots[f"robot_{i}"] = robot_type(room, speed, capacity)
    #can keep track of individual robots using a dictionary

    total_tiles = float(room.get_num_tiles())
    cur_coverage = 0.0

    time = 0
    for _ in range(num_trials):
        # anim = ps3_visualize.RobotVisualization(num_robots, width, height, [], delay = 0.00005) 
        while cur_coverage < min_coverage:
            for robot in robots.keys():
                robots[robot].update_position_and_clean()
            # anim.update(room, robots.values())
            cur_coverage = room.get_num_cleaned_tiles() / total_tiles
            time += 1
        # anim.done()
    return time / num_trials

# print ('avg time steps: ' + str(run_simulation(1, 1.0, 1, 5, 5, 3, 1.0, 50, FaultyRobot)))
# print ('avg time steps: ' + str(run_simulation(1, 1.0, 1, 10, 10, 3, 0.8, 50, FaultyRobot)))
# print ('avg time steps: ' + str(run_simulation(1, 1.0, 1, 10, 10, 3, 0.9, 50, FaultyRobot)))
# print ('avg time steps: ' + str(run_simulation(1, 1.0, 1, 20, 20, 3, 0.5, 50, FaultyRobot)))
# print ('avg time steps: ' + str(run_simulation(3, 1.0, 1, 20, 20, 3, 0.5, 50, FaultyRobot)))
# StandardRobot() EmptyRoom()
    # avg time steps: 5.72
    # avg time steps: 11.86
    # avg time steps: 13.42
    # avg time steps: 25.28
    # avg time steps: 8.44
# FaultyRobot() EmptyRoom()
    # avg time steps: 7.72
    # avg time steps: 12.14
    # avg time steps: 14.4
    # avg time steps: 27.36
    # avg time steps: 9.42
# === Problem 6
#
...
#       of a 20x20 room?
'''
The StandardRobot does better at every amount of robots tested.
However, it is very negligible after 4 robots are used. Even with just 2 robots,
the difference becomes small (around 10 time steps difference between ~50 for standard and ~60 for faulty).
'''
#
#
...
#
'''
The simulated performance of the StandardRobots are essentially constant relative to the aspect ratio of the room.
However, the simulated FaultyRobots are linearly proportional to the aspect ratio of the room
for ratios greater than 2. For aspect ratios less than or equal to 2, it is also constant performance.
Regardless, it is slower than the StandardRobots.
'''
#
...
# run_simulation(2, 1.0, 1, 10, 15, 3, 0.8, 200, StandardRobot)
# graph = RectangularRoom(15,25,5)
# print(graph.tiles)
